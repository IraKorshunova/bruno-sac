...
if __name__ == "__main__":
    ...
    train_params = defaults.cartpole_params_train
    test_params = defaults.cartpole_params_test

    train_envs_names, test_envs_names = env_utils.get_envs(env_type='CartpoleEnv',
    ...
    test_envs = [env_utils.initialize_envs([name], 42 + i)[0] for i, name in enumerate(test_envs_names)]
    ...
    env_params_dim = 2
    extra_dims = 0  # extra latent dimensions
    seq_len = 50
    context_len = [20, 30, 40]
    batch_size = 10
    ...
    if args.train:
        ...
        algorithm.train(max_episodes=2000, n_exploration_episodes=200,
                        min_collected_episodes=50,
                        max_episode_length=200, max_test_episode_length=200,
                        n_updates=200,
                        batch_size_episodes=batch_size, batch_seq_len=seq_len,
                        replay_buffer=replay_buffer,
                        n_save_iter=20, plot_n_steps=0, plot_diagnostics=False,
                        save_dir=save_dir)
    else:
        ...
        algorithm.test(max_episode_length=1000, train_iteration=0, n_episodes=10, save_dir=save_dir,
                       plot_n_steps=100, dump_data=True, plot_diagnostics=False)
